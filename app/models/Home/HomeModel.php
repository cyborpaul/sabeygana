...
class HomeModel extends Model
{
  ...
  */
  ...
  public function getPreguntas($id){
    return $this->db->query("SELECT * FROM inventalogame_preguntas WHERE niv_int_id='1' ORDER BY rand() LIMIT 3 ")->fetch_array(MYSQLI_ASSOC);
  }
  ...
  public function actualizarMovimiento($request_params){
    $id = $request_params['id_jugador'];
    $movimiento=$request_params['exec'];
    $concepto="Aplicativo Sabe Y Gana";
    $zonahoraria="-5";
